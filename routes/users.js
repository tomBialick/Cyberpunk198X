...
router.post('/auth', function(req, res, next) {
  ...
  db.query('SELECT * FROM USERS WHERE USERNAME = $1', [username]).then(data => {
    if (!data[0]) {
      ...
      req.session.save();
      res.status(201).json({body: {results: {status: "Authorized", username: data[0].username, role: data[0].role}}});
    }
    else {
  ...
  if (!req.query.token) {
    ...
    db.query('SELECT * FROM USERS WHERE USERNAME = $1', [req.session.user]).then(data => {
      if (!data[0]) {
        ...
        res.status(200).json({body: {results: {status: "Authorized", username: data[0].username, role: data[0].role}}});
      }
      else {
    ...
    db.query('SELECT * FROM USERS WHERE USERNAME = $1', [username]).then(data => {
      if (data[0]) {
        ...
        db.query('SELECT MAX(ID) FROM USERS').then(results => {
          ...
          db.query('INSERT INTO USERS (ID, USERNAME, PASSWORD, ROLE) VALUES ($1, $2, $3, $4)', [user_id, username, password, role]).then(results => {
